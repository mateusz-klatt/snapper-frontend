...
test.describe('order flows', () => {
  test('places a limit order via NewOrderModal happy path', async ({ browser }) => {
    ...
    // Default order type is `limit` — sticking with it keeps the
    // path inside pointer-driven inputs only. Driving the Radix
    // Select dropdown to `market` would require a hit-test bypass:
    // the host Modal uses native `<dialog>.showModal()` which puts
    // the dialog on the browser top layer, while Radix Select
    // renders its dropdown via Portal into document.body — i.e.
    // underneath the dialog backdrop. A real user clicks through
    // fine because the browser hit-tests against the visible pixel;
    // in Playwright the synthetic click is intercepted by the
    // top-layer dialog backdrop. The market path can return when
    // Modal moves the dropdown content into the dialog subtree
    // (separate v1.4.x). Limit + price exercises the same envelope
    // shape through to POST /api/orders.
