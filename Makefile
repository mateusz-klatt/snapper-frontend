...
.PHONY: help install dev build preview lint lint-fix format format-fix typecheck test test-watch cov dead-code dead-code-fix gen icons-vendor check-all fix-all clean

help:
	@echo "Snapper Frontend — Makefile targets"
	@echo ""
	@echo "  install        Install dependencies (pnpm install)"
	@echo "  dev            Start vite dev server on :5173"
	@echo "  build          Production build"
	@echo "  preview        Serve production build locally"
	@echo ""
	@echo "  lint           ESLint check"
	...
	@echo ""
	@echo "  gen            Regenerate types from a running backend at :8000"
	@echo "  icons-vendor   Re-vendor crypto + flag SVGs into public/icons/"
	@echo ""
	@echo "  check-all      lint + format + typecheck + test + dead-code + cov"
...
check-all: lint format typecheck test dead-code cov
